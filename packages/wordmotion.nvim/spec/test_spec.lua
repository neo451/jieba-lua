...
describe("test motion", function()
    ...
    it("tests from path", function()
        assert.are.equal(#motion.lines, 4)
    end)
    it("tests utf8", function()
        ...
        assert.are.equal(pos[1], 1)
        assert.are.equal(pos[2], 0)
        pos = motion:get_character(true, { 4, 24 })
...
describe("test (/)/g(/g)", function()
    ...
    local motion = Motion:from_path(txt)
    it("tests cut sentences", function()
        local line = motion.lines[3]
        local sentences = motion:cut(line)
        assert.are.equal(#sentences, 6)
    end)
    it("tests )", function()
        local pos = motion:get_position(1, true, { 3, 0 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 7)
        pos = motion:get_position(1, true, { 3, 7 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 18)
        pos = motion:get_position(1, true, { 3, 18 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 27)
    end)
    it("tests g)", function()
        local pos = motion:get_position(1, false, { 3, 7 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 14)
        pos = motion:get_position(1, false, { 3, 14 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 24)
    end)
    it("tests (", function()
        local pos = motion:get_position(-1, true, { 3, 7 })
        assert.are.equal(pos[1], 3)
        assert.are.equal(pos[2], 0)
    end)
    it("tests g(", function()
